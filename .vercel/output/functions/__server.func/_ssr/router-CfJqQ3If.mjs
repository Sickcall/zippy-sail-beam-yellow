...
import { t as Route$10 } from "../_code-pY51TUGT.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/router-CfJqQ3If.js
...
var styles_default = "/assets/styles-DGkA2eCm.css";
...
function hasDatabaseUrl() {
	const raw = typeof process !== "undefined" ? process.env.DATABASE_URL : void 0;
	return Boolean(raw && raw.trim());
}
function memGet(code) {
	return store().rooms.get(code);
}
function memPut(code, state, version) {
	const s = store();
	const row = {
		state,
		version,
		actions: s.rooms.get(code)?.actions ?? [],
		updatedAt: Date.now()
	};
	s.rooms.set(code, row);
	return row;
}
function memAction(code, payload) {
	const s = store();
...
function memAck(code, upTo) {
	const row = store().rooms.get(code);
	if (!row) return;
	row.actions = row.actions.filter((a) => a.id > upTo);
}
async function getSharedSql() {
	if (!hasDatabaseUrl()) return null;
	try {
		const { getSql } = await import("./db-J8xldJq-.mjs");
		const sql = await getSql();
		const s = store();
		s.schemaReady ??= (async () => {
...
async function handleTableRelay(request) {
	try {
		...
		if (request.method === "GET" && url.searchParams.get("health") === "1") return json({
			ok: true,
		...
		if (request.method === "GET") {
			...
			if (mem && mem.version > 0 && mem.state != null) {
				version = mem.version;
				state = mem.state;
				actions = mem.actions.filter((a) => a.id > since).slice(0, 100).map((a) => ({
					id: a.id,
					payload: a.payload
				}));
			}
			if (sql) try {
				if (Math.random() < .05) await sql.query(`DELETE FROM table_actions WHERE created_at < now() - interval '30 minutes'`);
				...
				const actionRows = await sql.query(`SELECT id, payload FROM table_actions WHERE code = $1 AND id > $2 ORDER BY id LIMIT 100`, [code, since]);
				if (actionRows.length) {
					const byId = /* @__PURE__ */ new Map();
					for (const a of actions) byId.set(a.id, a.payload);
					for (const a of actionRows) byId.set(Number(a.id), a.payload);
					actions = [...byId.entries()].sort((a, b) => a[0] - b[0]).map(([id, payload]) => ({
						id,
						payload
					}));
				}
			...
			if (!state || version === 0) return json({
				state: null,
				version: 0,
				actions: [],
				db: hasDatabaseUrl() ? "postgres" : "memory",
			...
			return json({
				state: since === 0 || version > since ? state : null,
				version,
				actions,
				db: hasDatabaseUrl() ? "postgres" : "memory",
				exists: true
...
//#endregion
export { getRouter, getSql as i, ensureDbReady as n, getPglite as r, dbSource as t };
